...
class WC_Toffi_Onramp_Init
{
	...
	public static function install_woocommerce_core_notice()
	{
		echo '<div class="notice notice-error is-dismissible">
		  <p>' . __('The WooCommerce Invoice Gateway extension requires that you have the WooCommerce core plugin installed and activated.', 'toffi-onramp-source') . ' <a href="https://wordpress.org/plugins/woocommerce/" target="_blank">' . __('Install WooCommerce', 'toffi-onramp-source') . '</a></p>
		</div>';
	}
	...
	{
		$action_links = array(
			'settings' => '<a href="' . admin_url('admin.php?page=wc-settings&tab=checkout&section=toffi-onramp-source') . '" title="' . esc_attr(__('View WooCommerce Settings', 'toffi-onramp-source')) . '">' . __('Settings', 'toffi-onramp-source') . '</a>',
		);

		return array_merge($action_links, $links);
	}
}
WC_Toffi_Onramp_Init::init();

add_filter('woocommerce_payment_gateways', 'toffi_onramp_add_gateway_class');
function toffi_onramp_add_gateway_class($gateways)
{
	$gateways[] = 'WC_toffi_onramp_gateway';
	return $gateways;
}

add_action('plugins_loaded', 'toffi_onramp_init_gateway_class');
...
function toffi_onramp_init_gateway_class()
{
	class WC_toffi_onramp_gateway extends WC_Payment_Gateway
	{
		public function __construct()
		{
			...
			$this->method_description = 'Toffi Onramp Payment Gateway for Credit Card Payments';
			$this->enabled = 'yes';
		...
		public function init_form_fields()
		{
			...
			$this->form_fields = array(
				...
				'title' => array(
					...
					'default' => _x('Credit Cards', 'Credit Cards', 'toffi-onramp-source'),
					'desc_tip' => true,
				),
				'description' => array(
					...
					'default' => __('Pay using Credit Cards.', 'toffi-onramp-source'),
					'desc_tip' => true,
				)
			);
		}
